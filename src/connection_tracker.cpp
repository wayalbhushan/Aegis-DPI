...
Connection* ConnectionTracker::getConnection(const FiveTuple& tuple) {
    ...
    if (rev != connections_.end()) return &rev->second;
...
std::vector<Connection> ConnectionTracker::getAllConnections() const {
    ...
    for (const auto& pair : connections_) result.push_back(pair.second);
    return result;
}
...
void ConnectionTracker::forEach(std::function<void(const Connection&)> callback) const {
    for (const auto& pair : connections_) callback(pair.second);
}
...
std::string GlobalConnectionTable::generateReport() const {
    ...
    ss << "\n╔══════════════════════════════════════════════════════════════╗\n"
       ...
       << "╠══════════════════════════════════════════════════════════════╣\n"
       << "║ Active Connections:     " << std::setw(10) << stats.total_active_connections << "                         ║\n"
       << "║ Total Connections Seen: " << std::setw(10) << stats.total_connections_seen << "                         ║\n"
       << "╠══════════════════════════════════════════════════════════════╣\n"
